...
docker create \
  ...
  -e GROUP_NAME=linuxserver.io `#optional` \
  -e BAD_USER_NAME=false `#optional` \
  -p 2222:2222 \
...
services:
  openssh-server:
    ...
    environment:
      ...
      - GROUP_NAME=linuxserver.io #optional
      - BAD_USER_NAME=false #optional
    volumes:
...
| `-e GROUP_NAME=linuxserver.io` | Optionally specify a group name (Default:`linuxserver.io`) |
| `-e BAD_USER_NAME=fase` | Optionally allow names that do not conform to standards (with upper case, dot, long names, etc.) (Default:`false`) |
